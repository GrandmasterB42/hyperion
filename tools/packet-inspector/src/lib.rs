...
#![expect(
    ...
    reason = "todo: we should double check no wrapping"
)]
#![expect(
...
use valence_protocol::{
    ...
    packets::{
        ...
        login::{LoginCompressionS2c, LoginDisconnectS2c, LoginHelloS2c, LoginSuccessS2c},
    },
    text::{Color, IntoText, color::NamedColor},
};
        ...
        let s2c = tokio::spawn(async move {
            loop {
                ...
                if state == PacketState::Login {
                    ...
                    if extrapolate_packet::<LoginSuccessS2c<'_>>(&packet).is_some() {
                ...
                if state == PacketState::Login
                    && extrapolate_packet::<LoginHelloS2c<'_>>(&packet).is_some()
                {
