...
use hyperion_proxy_proto::{
    Channel, ChannelId, ConnectionId, Crypto, ProxyId,
    packets::{self, p2s::ArchivedProxyToServerMessage},
};
...
use valence_protocol::{VarInt, bytes, packets::play};

use crate::{Compose, IoBuf, decoder::PacketDecoder, lookup::StreamLookup, packet_state};
                    ...
                    command_channel.push(move |world: &mut World| {
                        ...
                        for channel in query.iter(world) {
                            ...
                            tx.send(IoBuf::encode_proxy_message(
                                &packets::s2p::ServerToProxyMessage::AddChannel(
                                    packets::s2p::AddChannel {
