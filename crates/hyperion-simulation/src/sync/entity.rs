...
pub(crate) fn entity_metadata_sync(
    compose: Res<'_, Compose>,
...
pub(crate) fn active_animation_sync(
    compose: Res<'_, Compose>,
...
pub(crate) fn sync_player_entity(
    compose: Res<'_, Compose>,
    blocks: Res<'_, Blocks>,
    mut query: Query<
        '_,
    ...
    >,
    mut event_writer: MessageWriter<'_, message::HitGroundEvent>,
...
pub(crate) fn update_projectile_positions(
    ...
    >,
    mut projectile_block_writer: MessageWriter<'_, message::ProjectileBlockEvent>,
    mut projectile_entity_writer: MessageWriter<'_, message::ProjectileEntityEvent>,
    ...
    for (arrow_entity, owner) in arrow_query.iter() {
        ...
        match get_first_collision(ray, &index, &blocks, query_set.p1(), Some(owner.entity)) {
            Some(Either::Left(entity)) => {
                // send event
                projectile_entity_writer.write(message::ProjectileEntityEvent {
                    client: entity,
            ...
            Some(Either::Right(collision)) => {
                // send event
                projectile_block_writer.write(message::ProjectileBlockEvent {
                    collision,
                    projectile: arrow_entity,
                });
            }
