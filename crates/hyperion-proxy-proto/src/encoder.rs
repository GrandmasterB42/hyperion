...
use anyhow::ensure;
use hyperion_data::{Buf, ScratchBuffer};
use tracing::trace;
use valence_protocol::{CompressionThreshold, Encode, VarInt};

use crate::packets::PacketBundle;

static MAX_PACKET_SIZE: usize = valence_protocol::MAX_PACKET_SIZE as usize;
...
impl PacketEncoder {
        ...
        P: PacketBundle,
    {
        ...
        if data_len > threshold {
            ...
            let data_len = VarInt(data_len.cast_signed() as i32);

            let packet_len = data_len.written_size() + compressed.len();
            #[expect(clippy::cast_possible_truncation)]
            let packet_len = VarInt(packet_len.cast_signed() as i32);
        ...
        #[expect(clippy::cast_possible_truncation)]
        let packet_len =
            VarInt((DATA_LEN_0_SIZE.cast_signed() as i64 + data_len.cast_signed()) as i32); // packet_len.written_size();
